...
Nous constatons en particulier que dans le cas  $E_{\rm ecal} = 0$ (Figure \ref{courbeLR}), la courbe ne passe pas par le le coeur du nuage de point à  faible  $E_{\rm hcal}$. 
...
La Figure \ref{LR_ecaliboveretrue_ecal_hcal}, nous montre que la régression linéaire est mauvaise à  faible $E_{\rm hcal}$ car en moyenne, $E_{\rm calib}/E_{\rm true}$ n'est pas proche de $1$. Plus intéressant, la figure de droite met en avant les non-linéarités du nuage de point : en effet il y a clairement une zone limitée où le modèle linéaire semble fonctionner, mais partout ailleurs, les points sont éloignés du plan.  

La Figure \ref{LR_ecaliboveretrue_etrue} montre que la calibration n'est pas bonne : $E_{\rm calib}/E_{\rm true}$  est trop écarté de 1, à part autour de $E_{\rm true} = \SIUnits{100}{\giga\electronvolt}$. Nous constatons que à haute énergie, la distribution dessine une asymptote. Cela est en fait un artefact lié à la simulation car les particules sont limités à $E_{\rm true} = \SIUnits{200}{\giga\electronvolt}$, donc nous ne retrouvons pas des points de part et d'autre de la ligne $E_{\rm calib}/E_{\rm true} = 1$. On ne peut donc rien donner de la courbe verte qui tente de suivre la distribution à haute énergie ($\approx \SIUnits{150}{\giga\electronvolt}$). Cela est valable pour toutes les figures qui présenteront E_{\rm calib}/E_{\rm true}$ en fonction de $E_{\rm true}$.
...
Cette fois ci, nous avons dans le plan $E_{\rm ecal } = 0$ (\textit{cf.} Figure \ref{calibrationKNNGC}), la courbe de calibration qui passe par le coeur de la distribution à faible $E_{\rm hcal}$ grâce au nettoyage gaussien.
...
La Figure \ref{ecaliboveretrueKNNGF} nous montre que la calibration correcte pour les particules dont l'énergie vraie est comprise entre $20$ et $140 \giga\electronvolt$.
...
La Figure \ref{comparaison1} montre les courbes $E_{\rm calib}/E_{\rm true}$ moyens en fonction de $E_{\rm hcal}$ dans le plan $E_{\rm ecal} = 0$ pour toutes les méthodes de calibration. Il apparaît alors que dans ce plan, la méthode des plus proches voisins avec un nettoyage gaussien donne le meilleur résultat. Celle avec le fit gaussien donne des résultats assez similaires. Il est à noté que pour des particules de bas $E_{\rm hcal}$ $(< \SIUnits{10}{\giga\electronvolts}$), l'énergie calibrée sous évalue en moyenne l'énergie vraie pour ces deux méthodes. Par la suite, il faudra trouver un moyen de corriger cela.

\begin{figure}[!h]
\centering
\includegraphics[width=\textwidth]{images/pictures/comparisons/comparison1.eps}
\caption{$E_{\rm calib}/E_{\rm true}$ moyens en fonction de $E_{\rm hcal}$ pour les particules n'interagissant pas avec le ECAL. Chaque point d'une courbe correspond à la moyenne d'un fit gaussien de points $E_{\rm calib}/E_{\rm true}$ proches de l'abscisse correspondante.}
\label{comparaison1}
\end{figure}

La Figure \ref{comparaison1} illustre $E_{\rm calib}/E_{\rm true}$ dans le plan $(E_{\rm ecal},E_{\rm hcal})$ pour toutes les méthodes de calibration. Les méthodes des plus proches voisins avec le nettoyage gaussien et le fit gaussien sont les meilleures proposées. A noter que l'on observe une sous évaluation de l'énergie vraie par l'énergie calibrée à bas $E_{\rm ecal}$, mais la calibration reste bonne dans le plan  $(E_{\rm ecal},E_{\rm hcal})$.
...
La Figure \ref{comparaison3} nous montre pour les sous-figures du haut $E_{\rm calib}/E_{\rm true}$ moyens en fonction de $E_{\rm true}$ dans les cas $E_{\rm ecal} = 0$ et $E_{\rm ecal} \neq 0$. Ces figures illustrent encore que les deux dernières méthodes sont les meilleures et permettent une bonne calibration pour des particules d'énergie vraie $E_{\rm true}$ comprises entre $10$ et $\SIUnits{120}{\giga\electronvolts}$ dans le premier cas et entre $20$ et $\SIUnits{140}{\giga\electronvolts}$ dans le second.

Les sous-figures du bas présente l'écart-type des $E_{\rm calib}/E_{\rm true}$ moyens en fonction de $E_{\rm true}$ dans les cas $E_{\rm ecal} = 0$ et $E_{\rm ecal} \neq 0$. Ces écarts-types sont des fonctions décroissantes de l'énergie vraie, cela signifie que la précision augmente avec l'énergie vraie.
\begin{figure}[!h]
\centering
...
En bas à gauche : écart type du fit gaussien correspondant.
...
En bas à droite : écart type du fit gaussien correspondant.}
\label{comparaison3}
\end{figure}
...
\newpage
\section{Conclusion}
Les méthodes des plus proches voisins avec un nettoyage gaussien ou avec un fit gaussien permettent de calibrer l'énergie des particules pour estimer l'énergie vraie des particules en fonction de leur dépôts d'énergie dans les calorimètres ECAL et HCAL. Cependant, elles restent à améliorer, notamment pour améliorer la calibration pour les particules de faible énergie. 

D'autres points seront à développer :
\begin{itemize}
\item comment extrapoler une énergie de calibration pour des particules de plus hautes énergies ?
\item il faudrait prendre en compte plus d'informations comme par exemple l'angle de la trajectoire des particules.
\item il faudrait comparer avec la calibration faite jusqu'à présent qui elle utilisent une fonction paramétrée. \cite{PF}
\end{itemize}
