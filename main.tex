...
\item d'un calorimètre électromagnétique (ECAL) \cite{ECAL1}, constitué d'un cristal de tungstate de plomb, permettant de collecter les dépôts d'énergie des particules, principalement électrons et photons, mais aussi hadrons chargés et neutres);
...
Une technique de calibration consiste à déterminer l'énergie calibrée par une fonction linéaire de l'énergie lue dans le ECAL et de celle lue dans le HCAL. Cette méthode sera présentée dans la section \ref{LR}.
...
\label{LR}
\subsection{Principe de la calibration}
- Un première calibration\\
\begin{equation}
