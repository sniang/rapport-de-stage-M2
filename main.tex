...
Après avoir permis la découverte expérimentale du boson de Higgs en 2012 \cite{HiggsAtlas,HiggsCMS}, les expériences généralistes ATLAS et CMS installées sur le LHC du CERN, sont toujours en place dans l'optique de découvrir de la nouvelle physique au delà du modele standard. \\

Les détecteur ATLAS et CMS sont basés sur les mêmes principes : ils disposent tous deux d'une structure cylindrique en couches placées autour du point d'interaction où les collisions proton-proton sont réalisées.\\
Le but de ces détecteur étant de déterminer le type, l'énergie ainsi que la direction des particules de l'état final de la collision, pour pouvoir en mesurer les propriétés, et par exemple determiner si une nouvelle particule instable a été produite. \\

Nous allons alors nous intéresser plus spécifiquement au détecteur CMS \cite{CMS}, celui-ci dispose  :
\begin{itemize}
\item d'un champ magnétique, pour séparer les dépôts d'énergie du calorimètre des particules chargées et neutres dans les jets
\item d'un traceur, pour reconstruire la trajectoire des particules chargées
\item d'un calorimètre éléctromagnétique (ECAL) \cite{ECAL1,ECAL2}, constitué d'un cristal de tungstate de plomb, permettant de dégager des dépôts d'énergie à partir de particules dans des jets (hadrons chargés, hadrons neutres et photons) clairement séparés l'un de l'autre jusqu'à un jet pT de l'ordre de \SIUnits{1}{\tera\electronvolt}. L'identification efficace du photon résultant, couplée à la résolution élevée de l'énergie ECAL, permet une excellente mesure d'environ $\tilda 25\%$ de l'énergie du jet
\item d'un calorimètre hadronique (HCAL) \cite{HCAL}, composé de plusieurs couches d'amortisseurs en laiton et de carreaux scintillateurs en plastique, avec une segmentation grossière mais suffisante pour séparer les dépôts d'énergie hadron chargés et neutres dans des jets jusqu'à de $\SIUnits{200-300}{\giga\electronvolt}$, ce qui permet de reconstruire les $10\%$ restants de l'énergie du jet, bien qu'avec une modeste résolution
\item d'un système de suivi des muons, fournissant une identification, indépendamment des particules environnantes.
\end{itemize}
...
Détaillons alors le comportement des particules stables dans le détecteur :
\begin{itemize}
    \item $e^+,e^-$ :
        \begin{itemize}
        		\item suivis dans le traceur
    ...
    \item $\mu^+,\mu^-$ :
        \begin{itemize}
        		\item suivis dans le traceur
		\item traverse ECAL, HCAL
		\item système de suivis des muons
        \end{itemize}
\end{itemize}
\`A noter que dans notre étude seul les hadrons chargés nous intéressent. \\
Toutes ces informations vont permettre par la suite permettre de reconstruire toute la décomposition qui s'effectue dans le détecteur, ce que l'on appelle le \textit{Particle Flow (PF)}.\\

Cette complexité de détection, plus le fait que les dépôts d'énergies sont reliés aux sections efficaces des particules en mises en jeu font que la réponse des calorimètres n'est pas linéaire et que la somme des énergies dans les calorimètres ne correspond pas à l'énergie de l'événement incident, d'où la nécessité d'une calibration.\\

Une technique classique de calibration consiste à déterminer l'énergie calibrée par une fonction linéaire de l'énergie lue dans le ECAL et de celle lue dans le HCAL (malgré ce qui fût dit plus haut). Nous reviendrons en détail sur cette méthode dans la section \ref{LR}.\\
De plus, ce rapport présente de nouvelles techniques de calibration qui permettent de prendre en compte la non-linearité des calorimètres. Ces techniques sont présentées dans les sections \ref{CL}, \ref{KNN}, \ref{KNNGC} et enfin \ref{KNNGF}.\\
